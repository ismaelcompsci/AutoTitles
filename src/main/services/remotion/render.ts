import { renderMedia, RenderMediaOnProgress, selectComposition } from '@remotion/renderer'
...
if (app.isPackaged) {
  ...
  binariesDirectory = path.join(app.getAppPath().replace('app.asar', 'app.asar.unpacked'), pathName)
  console.info(`Binaries directory: ${binariesDirectory}`)
}
...
export default async function render(
  ...
  console.info(`Bundle location: ${bundleLocation}`)
  ...
  console.info(`Composition selected: ${composition.id}`)

  // Get the downloads folder path
  const homeDirectory = os.homedir()
  const downloadsFolderPath = path.join(homeDirectory, 'Downloads/output-remotion.mp4')
  console.info(`Downloads folder path: ${downloadsFolderPath}`)

  // Render the video with input props
  console.info(`Rendering video: ${compositionId}.mp4`)
  await renderMedia({
    composition,
    serveUrl: bundleLocation,
    codec: 'h264',
  ...
  console.info(`Video rendered: ${compositionId}.mp4`)

  shell.showItemInFolder(downloadsFolderPath)
  console.info('Shell opened /downloads folder')
}
