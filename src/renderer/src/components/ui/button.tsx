  ...
  'transition-colors select-none font-medium border border-transparent flex justify-center items-center gap-0.5 max-w-full disabled:bg-gray-100 disabled:text-gray-700 disabled:border-gray-400 disabled:cursor-not-allowed',
  {
    variants: {
      variant: {
        ...
        error: 'bg-red-800 border-red-800 hover:bg-red-900 hover:border-red-900',
        warning:
    ...
    return (
      <Comp
        className={cn(
          ...
          (shape === 'square' || shape === 'circle') && 'aspect-square h-[unset]',
        ...
        {...props}
      >
        {loading ? <Loader2 className="animate-spin" size={size === 'large' ? 24 : 16} /> : prefix}
